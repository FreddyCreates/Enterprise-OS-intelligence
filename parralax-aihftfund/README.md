...
| [`MODEL_ARCHITECTURES.md`](./MODEL_ARCHITECTURES.md) | **Three architecture specs.** `VATES-8B` (7.91B params · signal generation, ≤100ms · consumed by AUGUR/PROPHET); `AUSPEX-14B` (14.02B params · observation and world-model, ≤500ms · consumed by VIGIL/CEREBEX); `ORACULUM-20B` (19.90B params · council-level reasoning, ≤3s · consumed by ARCHON/ARBITER). All decoder-only LLaMA-family transformers with SwiGLU, RMSNorm, RoPE, GQA (AUSPEX/ORACULUM), shared 32K BPE vocab. Latin names in the divination register matching AUGUR/PROPHET. Parameter counts land within ±2% of the label. |
